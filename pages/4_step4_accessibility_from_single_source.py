...
def page_4_accessibility(
    ...
    # start building network and query the shortest paths
    if (st.button("Start analysis & plot results") or
        ...
        one_source_dists, one_source_paths = GRAPH_OBJ.query_origin_stop_time(
            stops_df=stops,
            stop_id=stop_id,
        ...
        m = folium_plots.plot_isochrone_combined_plot(
            acc_times,
            stops_gdf,
        )
...
if GTFS_OBJ is not None and GRAPH_OBJ is not None:
    ...
    m = page_4_accessibility(stops, stop_id, depart_hr, max_tt)
    if m is not None:
