...
The original networkx digraph object is augmented in the class for fast searching...
"""
...
class GTFSGraph:
    def __init__(self):
        self.G: nx.DiGraph = nx.DiGraph()  # spatial temporal graph
        self.G_reversed: None | nx.DiGraph = None
        # augmented data structure mapping station to all its nodes
    ...
    def add_skeleton_nodes(self, stop_dict, times_info):
        ...
        nodes_info = [
            (f"{stop_id}_{time_info}", stop_dict)
    ...
    def add_edge(
        ...
        """
        If t1=0, t2=0,
            then in fact two new nodes are not recorded in self.nodes_time_map
        Strategy for now:
            only nodes with transit arrived are recorded in self.nodes_time_map
        """
        # print("stop_a, stop_b: ", stop_a, stop_b)
    ...
    def add_hyper_nodes(self):
        ...
        for stop_id in stops:
            ...
            origins = [f"{stop_id}_{t}" for t in ts]
            for ori in origins:
    ...
    def add_edges_walkable_stops(
        ...
        for stop_id in self.nodes_time_map:
            ...
            nei_int_ids = stops_b.loc[stops_b["stop_id"] == stop_id]['neighbors'].iloc[0]
            nei_ids = stops_b.iloc[nei_int_ids]["stop_id"].tolist()
            # get index ids???
            # print("stop_id:", stop_id)
            # print("nei_int_ids:", nei_int_ids)
            ...
            for i in range(len(ts)):
                ...
                for j, nei_id in enumerate(nei_ids):
                    ...
                    if nei_id != stop_id:
                        self.add_edge(
                            stop_id, nei_id,
                            node_a=f"{stop_id}_{t0:.0f}",
                            node_b=f"{nei_id}_{t_end:.0f}",
                            t1=0, t2=t_end,
    ...
    def query_origin_stop_time(
        ...
        nei_int_idxs = one_stop_df["neighbors"]  # all neighbors index

        nei_dists = one_stop_df["dists"]  # distance in miles

        print("nei_int_idxs: ", nei_int_idxs)

        nei_stop_ids = stops_df.iloc[nei_int_idxs]["stop_id"]  # all neighbors IDs
        ...
        for i, sid in enumerate(nei_stop_ids):
            # add links
            self.G.add_edge(
                nodes_id_origin[i], nodes_id_next[i],
                **{"tt": wait_mins[i],
                   "wt": wait_mins[i],
                   "mode": "wait"}
            )
    ...
    # query shortest path
    def query_destination_stop_time(
            self,
        ...
        # get all proposed starting nodes
        nodes_id_origin = [
            f"{stop_id}_{depart_mins[i]:.0f}"
            for i, stop_id in enumerate(nei_stop_ids)
        ]

        # from stop to prev arrived vehicle
        prev_mins = np.array(
            [self.find_closest_prev_time(stop_id, depart_mins[i])
             for i, stop_id in enumerate(nei_stop_ids)]
        )
        wait_mins = arrive_min - prev_mins  # minus the queried starting time here
        # get all proposed starting nodes to take transits
        nodes_id_prev = [
            f"{stop_id}_{prev_mins[i]:.0f}"
            for i, stop_id in enumerate(nei_stop_ids)
        ]
        # add link (no need to track for this in self.nodes_time_map)
        # just wait at the stop for transit to arrive
    ...
    def query_destination_stops_time(
            self,
            stops_df: pd.DataFrame,
            stop_ids: list[str],
            arrive_min: int,
        ...
        for stop_id in stop_ids:
            self.query_destination_stop_time(
                stops_df, stop_id, arrive_min, cutoff,
                walk_speed=walk_speed,
                renew_reversed=renew_reversed,
                run_dijkstra=False
            )
    ...
    def query_od_stops_time(
        ...
        node_id_origin = f"{stop_orig_id}_{depart_min:.0f}"
        node_id_dest = f"{stop_dest_id}_D"
        # add on edge...
        next_min = self.find_closest_next_time(stop_orig_id, depart_min)
        wait_min = next_min - depart_min
        node_id_next = f"{stop_orig_id}_{next_min:.0f}"
    ...
    def generate_reverse_network(self) -> None:
    ...
    def find_closest_next_time(self, stop_id, time_min) -> int:
        idx = self.nodes_time_map[stop_id].bisect_left(time_min)
        return self.nodes_time_map[stop_id][idx]  # return the next time

    def find_closest_prev_time(self, stop_id, time_min) -> int:
        idx = self.nodes_time_map[stop_id].bisect_left(time_min)
        return self.nodes_time_map[stop_id][idx - 1]  # return the next time

    # get the shortest travel times from one stop to the others
    def get_shortest_tts(
            self,
            one_source_results,
            filter_failed=False
    ) -> dict:
        ...
        for stop_id in stops:
            ...
            try:
                ...
                if filter_failed:
                    continue
                one_source_tts = -1  # placeholder for non-feasible trips
...
def plot_one_stop_over_time(G_subgraph: nx.DiGraph):
